...
def extract_pe_features(fpath):
    """
    Extracts features from a PE file and returns them as a dictionary.
    """
    res = {}
    pe = pefile.PE(fpath)
    ...
    res['SectionsMeanEntropy'] = sum(entropy)/len(entropy) if entropy else 0
    ...
    res['SectionsMeanVirtualsize'] = sum(virtual_sizes)/len(virtual_sizes) if virtual_sizes else 0
    res['SectionsMinVirtualsize'] = min(virtual_sizes) if virtual_sizes else 0
    res['SectionsMaxVirtualsize'] = max(virtual_sizes) if virtual_sizes else 0
    ...
    if resources:
        ...
        res['ResourcesMeanEntropy'] = res['ResourcesMinEntropy'] = res['ResourcesMaxEntropy'] = 0
        res['ResourcesMeanSize'] = res['ResourcesMinSize'] = res['ResourcesMaxSize'] = 0
...
if __name__ == '__main__':
    import sys
    if len(sys.argv) < 2:
        print("Usage: python Extract/PE_main.py <file_path>")
    else:
